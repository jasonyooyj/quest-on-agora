  ...
  return (
    <div className="min-h-screen bg-[#09090b] text-white selection:bg-primary/30">
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute bottom-[-10%] right-[-10%] w-[40%] h-[40%] bg-blue-500/10 rounded-full blur-[120px] animate-blob [animation-delay:2s]" />
        <div className="absolute top-[20%] right-[10%] w-[30%] h-[30%] bg-purple-500/5 rounded-full blur-[100px] animate-blob [animation-delay:4s]" />
        ...
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          <div className="flex items-center gap-6">
            <Link href="/instructor">
              <button className="w-10 h-10 rounded-full bg-white/5 border border-white/10 flex items-center justify-center hover:bg-white/10 transition-all active:scale-95 group">
          ...
          <div className="flex items-center gap-3">
            <Link href="/instructor/discussions/new">
              <button className="h-12 px-6 rounded-full bg-white text-black font-black text-sm flex items-center gap-2 hover:shadow-[0_0_20px_rgba(255,255,255,0.3)] transition-all active:scale-95">
      ...
      <main className="max-w-7xl mx-auto p-6 space-y-8 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          {[
            { label: '총 토론', value: discussions.length, icon: MessageSquare, color: 'text-blue-400', bg: 'bg-blue-400/10' },
            { label: '진행중', value: activeCount, icon: TrendingUp, color: 'text-emerald-400', bg: 'bg-emerald-400/10' },
            { label: '종료됨', value: closedCount, icon: CheckCircle2, color: 'text-zinc-400', bg: 'bg-zinc-400/10' },
            { label: '총 참여자', value: totalParticipants, icon: Users, color: 'text-purple-400', bg: 'bg-purple-400/10' },
          ].map((stat, i) => (
            <motion.div
              key={stat.label}
        ...
        <div className="space-y-6">
          <div className="flex items-end justify-between px-2">
            <div>
              <h2 className="text-2xl font-black text-white">내 토론 목록</h2>
              <p className="text-zinc-500 font-medium">생성된 모든 토론 세션입니다</p>
            </div>
            <div className="bg-white/5 px-4 py-2 rounded-full border border-white/5 text-[10px] font-black text-zinc-400 uppercase tracking-widest">
          ...
          <div className="grid gap-4">
            {loading ? (
              ...
              <motion.div
                ...
                className="glass-panel p-20 text-center border-white/5 bg-white/[0.01]"
              >
                ...
                <Link href="/instructor/discussions/new">
                  <button className="h-14 bg-white text-black font-extrabold rounded-full px-8 hover:shadow-[0_0_20px_rgba(255,255,255,0.3)] transition-all active:scale-95">
                    첫 토론 만들기
                  </button>
                </Link>
              </motion.div>
