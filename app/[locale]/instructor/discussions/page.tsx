    ...
    <div className="min-h-screen bg-white text-zinc-900 selection:bg-primary/30">
      ...
      <main className="max-w-7xl mx-auto p-6 space-y-8 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          {[
            { label: t('stats.totalSessions'), value: discussions.length, icon: MessageSquare, color: 'text-blue-500', bg: 'bg-blue-500/10' },
            { label: t('stats.activeSessions'), value: activeCount, icon: TrendingUp, color: 'text-emerald-500', bg: 'bg-emerald-500/10' },
            { label: t('list.status.closed'), value: closedCount, icon: CheckCircle2, color: 'text-zinc-500', bg: 'bg-zinc-500/10' },
            { label: t('stats.totalParticipants'), value: totalParticipants, icon: Users, color: 'text-purple-500', bg: 'bg-purple-500/10' },
          ].map((stat, i) => (
            <motion.div
              key={stat.label}
        ...
        <div className="space-y-6">
          <div className="flex items-end justify-between px-2">
            <div>
              <h2 className="text-2xl font-black text-zinc-900">{t('manage.title')}</h2>
