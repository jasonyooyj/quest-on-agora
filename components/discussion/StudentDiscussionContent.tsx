...
interface StudentDiscussionContentProps {
    discussionId: string
    userId: string
    isPreview?: boolean
    onBack?: () => void
...
export function StudentDiscussionContent({
    discussionId,
    userId,
    isPreview = false,
    onBack,
    ...
    galleryHref,
    submitHref
}: StudentDiscussionContentProps) {
    const t = useTranslations('Student.Dashboard.DiscussionDetail')
    const tPreview = useTranslations('Instructor.Preview')
    ...
    const isLoading = isDiscussionLoading || isParticipantsLoading
    ...
    const requestHelp = () => {
        if (!participant || isPreview) return
    ...
    const handleSubmitDiscussion = async () => {
        if (!participant || isSubmitting || isPreview) return
    ...
    }, [discussionId, locale, message, participant, queryClient, sending, t, isPreview])
        ...
        return (
            <div className="min-h-screen flex items-center justify-center">
                <div className="brutal-box p-8 max-w-md text-center">
                    ...
                    <button onClick={onBack} className="btn-brutal-fill mt-4">
                        {t('notFound.button')}
                    </button>
                </div>
            </div>
    ...
    const effectiveGalleryHref = galleryHref || `/student/discussions/${discussionId}/gallery`
    const effectiveSubmitHref = submitHref || `/student/discussions/${discussionId}/submit`
    ...
    return (
        <div className="min-h-screen flex flex-col bg-white text-zinc-900 selection:bg-primary/30 relative overflow-hidden">
            ...
            {/* Preview Mode Banner */}
            {isPreview && (
                <div className="bg-amber-50 border-b border-amber-200 px-4 py-3 flex items-center justify-center gap-3 relative z-50">
                    <AlertCircle className="w-5 h-5 text-amber-600" />
                    <span className="text-amber-800 font-bold text-sm">{tPreview('banner')}</span>
                    <button
                        onClick={onBack}
                        className="ml-4 px-4 py-1.5 bg-amber-600 text-white text-sm font-bold rounded-full hover:bg-amber-700 transition-colors"
                    >
                        {tPreview('exit')}
                    </button>
                </div>
            ...
            <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-xl border-b border-zinc-200">
                <div className="max-w-4xl mx-auto px-4 sm:px-6 h-16 sm:h-20 flex items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2 sm:gap-3 flex-nowrap shrink-0">
                        ...
                        {!participant?.isSubmitted && participant?.stance && !isPreview && (
                            <Link
                                href={effectiveSubmitHref}
            ...
            {/* Submit Confirmation Dialog - only shown for non-preview */}
            {!isPreview && (
            ...
            {/* Extension Request Dialog - only shown for non-preview */}
            {!isPreview && (
                    ...
                    <AlertDialogContent className="rounded-[2rem] border-zinc-200 max-w-md">
                        ...
                        <AlertDialogFooter className="gap-3 flex-col sm:flex-row">
                            ...
                            <AlertDialogAction
                                onClick={() => {
                                    setShowExtensionDialog(false)
                                    window.location.href = effectiveSubmitHref
                                }}
                                className="flex-1 h-12 rounded-full bg-gradient-to-r from-indigo-500 to-purple-600 font-bold"
                            >
            ...
            {/* Help Confirmation Modal - only shown for non-preview */}
            {!isPreview && (
